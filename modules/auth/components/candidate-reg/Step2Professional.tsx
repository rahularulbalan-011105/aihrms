      ...
      <section className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-display font-bold text-[15px] text-ink-900">Education Details</span>
          </div>
          <button type="button" className="text-[13px] font-semibold text-brand-600 hover:text-brand-700 flex items-center gap-1 transition">
            + Add Education
          </button>
        </div>
            ...
            <thead className="bg-ink-50/80 border-b border-ink-200">
              <tr>
                {["Degree / Course", "Institute / University", "Specialization", "Year of Passing", "Actions"].map((h) => (
                  <th key={h} className="px-4 py-3 text-left font-semibold text-ink-600">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {education.map((edu, i) => (
                <tr key={edu.id} className={`border-b border-ink-100 last:border-0 ${i % 2 === 1 ? "bg-ink-50/30" : "bg-white"}`}>
                  <td className="px-4 py-3 text-ink-800 font-medium">{edu.degree}</td>
                  <td className="px-4 py-3 text-ink-600">{edu.institution}</td>
                  <td className="px-4 py-3 text-ink-600">{edu.specialization}</td>
                  <td className="px-4 py-3 text-ink-600">{edu.yearOfPassing}</td>
      ...
      <section className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-display font-bold text-[15px] text-ink-900">Professional Experience</span>
          </div>
          <button type="button" className="text-[13px] font-semibold text-brand-600 hover:text-brand-700 flex items-center gap-1 transition">
            + Add Experience
          </button>
        ...
        <div className="space-y-3">
          {experience.map((exp) => (
            <ExperienceCard key={exp.id} exp={exp} onToggle={() => toggleExpand(exp.id)} />
          ))}
        </div>
      </section>
    ...
    <div className="rounded-xl border border-ink-200 overflow-hidden">
      ...
      <div className="px-5 py-4 bg-white flex items-start justify-between gap-4">
        <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-3">
          <div>
            <div className="text-[11px] text-ink-400 font-medium">Company</div>
            <div className="text-[13.5px] font-bold text-ink-900 mt-0.5">{exp.company}</div>
          </div>
          <div>
            <div className="text-[11px] text-ink-400 font-medium">Employment Type</div>
            <div className="text-[13.5px] font-semibold text-ink-700 mt-0.5">{exp.employmentType}</div>
          </div>
          <div>
            <div className="text-[11px] text-ink-400 font-medium">Duration</div>
            <div className="text-[13.5px] font-semibold text-ink-700 mt-0.5">{exp.startDate} – {exp.endDate}</div>
          </div>
          <div>
            <div className="text-[11px] text-ink-400 font-medium">Total Experience</div>
            <div className="text-[13.5px] font-semibold text-ink-700 mt-0.5">{exp.totalExperience}</div>
          </div>
        </div>
      ...
      {exp.expanded && (
        <div className="border-t border-ink-100 bg-ink-50/30">
          {/* Detail row */}
          <div className="px-5 py-3 grid grid-cols-3 gap-3 text-[13px]">
                  ...
                  <thead className="bg-ink-50 border-b border-ink-200">
                    <tr>
                      {["Project Details", "Role & Duration", "Team Size", "Technologies Used", ""].map((h, i) => (
                        <th key={i} className="px-3 py-2.5 text-left font-semibold text-ink-500">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
...
function ProjectRow({ proj }: { proj: Project }) {
  return (
    <tr className="border-b border-ink-100 last:border-0">
      ...
      <td className="px-3 py-3 text-center font-semibold text-ink-700 align-top">{proj.teamSize}</td>
      <td className="px-3 py-3 align-top">
      ...
      <td className="px-3 py-3 align-top">
        <div className="flex items-center gap-1.5">
          <button type="button" className="p-1 rounded hover:bg-brand-50 text-ink-400 hover:text-brand-600 transition"><EditIcon /></button>
          <button type="button" className="p-1 rounded hover:bg-red-50 text-ink-400 hover:text-red-500 transition"><TrashIcon /></button>
        </div>
      </td>
    </tr>
  );
