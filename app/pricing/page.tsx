...
type Tier = {
  ...
  highlight?: boolean;
  toneAccent: "purple" | "green" | "orange" | "blue";
  iconBg: string;
  icon: string;
};
...
const AGENCY_TIERS: Tier[] = [
  {
    name: "Starter", tagline: "Perfect for small teams",
    price: "₹2,999", cadence: "/month", billing: "Billed annually",
    features: [
      "Up to 3 Users",
    ...
    toneAccent: "purple", iconBg: "bg-brand-50", icon: "✈",
  },
  {
    name: "Professional", tagline: "Best for growing agencies",
    price: "₹7,999", cadence: "/month", billing: "Billed annually",
    features: [
      "Up to 15 Users",
    ...
    highlight: true,
    toneAccent: "purple", iconBg: "bg-brand-100", icon: "♛",
  },
  {
    name: "Business", tagline: "For established agencies",
    price: "₹15,999", cadence: "/month", billing: "Billed annually",
    features: [
      "Up to 50 Users",
      "2000 Resume Parses / month",
      "All AI Capabilities",
      "Job Posting (Unlimited)",
      "Advanced Analytics & Reports",
      "Team Collaboration",
    ...
    toneAccent: "green", iconBg: "bg-green-50", icon: "💼",
  },
  {
    name: "Enterprise", tagline: "For large organizations",
    price: "Custom Pricing", billing: "Tailored to your needs",
    features: [
      "Unlimited Users",
    ...
    toneAccent: "orange", iconBg: "bg-orange-50", icon: "🏢",
  },
];
        ...
        <div className="mx-auto max-w-[1400px] px-6 lg:px-10 pt-14 lg:pt-20 pb-12 relative">
          <div className="text-center">
            ...
            <p className="mt-5 text-ink-500 text-[15px] max-w-[640px] mx-auto leading-relaxed">
              Choose the perfect plan to power your hiring with AI.
            </p>
          </div>
          ...
          <div className="mt-8 flex justify-center">
            <div className="inline-flex p-1 rounded-xl border border-ink-200 bg-white">
              <ToggleBtn active={audience === "agencies"} onClick={() => setAudience("agencies")}>
                🏢 For Agencies / Recruiters
              </ToggleBtn>
              <ToggleBtn active={audience === "seekers"} onClick={() => setAudience("seekers")}>
                👤 For Job Seekers
              </ToggleBtn>
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-ink-100/50">
                <tr>
                  <th className="px-6 py-4 text-[13px] font-bold text-ink-900">
                    <span className="inline-flex items-center gap-2">⚙ Compare Plans</span>
                  </th>
              ...
              <tbody>
                {COMPARE_ROWS.map((r, i) => (
                  <tr key={r.feature} className={i % 2 === 1 ? "bg-ink-100/20" : ""}>
                    <td className="px-6 py-3 text-[13px] text-ink-700 flex items-center gap-2">
                      <FeatureGlyph name={r.feature} /> {r.feature}
                    </td>
                    <Cell v={r.starter} />
...
function ToggleBtn({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`px-5 py-2.5 rounded-lg text-[13px] font-semibold transition flex items-center gap-2 ${
        active ? "text-white shadow-sm" : "text-ink-700 hover:bg-ink-100"
      }`}
...
function TierCard({ t }: { t: Tier }) {
  const containerClass = t.highlight
    ? "rounded-2xl p-6 relative shadow-[0_18px_50px_-20px_rgba(109,76,255,0.35)]"
    : "card p-6";
  const containerStyle = t.highlight
    ? { background: "linear-gradient(180deg, #f4f1ff 0%, #ffffff 100%)", border: "2px solid #6D4CFF" }
    : undefined;

  return (
    <div className={containerClass} style={containerStyle}>
      {t.highlight && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 text-white text-[10px] font-bold tracking-wide uppercase px-3 py-1 rounded-full" style={{ background: "var(--gradient-brand)" }}>
          Most Popular
        </div>
      )}
      <div className={`w-12 h-12 rounded-full ${t.iconBg} flex items-center justify-center text-[18px] mb-4`}>
        {t.icon}
      </div>
      ...
      <div className="font-display text-[32px] font-extrabold leading-none">
        {t.price}
        {t.cadence && <span className="text-[14px] text-ink-500 font-semibold ml-1">{t.cadence}</span>}
      </div>
      ...
      <Link
        href={t.cta.href}
        className={`block text-center py-3 rounded-lg text-[14px] font-semibold transition ${
          t.highlight
...
function Cell({ v, highlight }: { v: string; highlight?: boolean }) {
  ...
  return (
    <td className={`px-4 py-3 text-[12.5px] text-center ${highlight ? "text-brand-700 font-semibold" : "text-ink-700"}`}>
      {isCheck ? (
        <span className="inline-flex w-6 h-6 rounded-full bg-green-50 text-green-700 items-center justify-center">✓</span>
      ) : isDash ? (
