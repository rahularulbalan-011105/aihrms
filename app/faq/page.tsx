...
const QUESTIONS: Q[] = [
  {
    q: "What is AI HRMS?",
    a: "AI HRMS is an AI-powered recruitment platform that helps job seekers find the right opportunities and helps agencies streamline hiring with AI-driven matching, automation, and insights.",
    cat: "general",
  },
  {
  ...
  {
    q: "Is AI HRMS suitable for both job seekers and recruitment agencies?",
  ...
  {
    q: "Can I try AI HRMS before subscribing?",
    a: "Absolutely — every plan includes a free trial. You can also book a live demo with our team to see how AI HRMS fits your hiring workflow before you commit.",
    cat: "pricing",
  },
  {
    q: "How secure is my data on AI HRMS?",
  ...
  {
    q: "What integrations does AI HRMS support?",
    a: "AI HRMS integrates with leading job boards, ATS systems, calendar tools, and communication platforms. Custom integrations are available on Business and Enterprise plans.",
    cat: "integrations",
  },
  {
  ...
  // Hidden until search expands them — counts still match the sidebar
...
const COUNTS: Record<Category, number> = {
  all: 12, general: 3, seekers: 3, agencies: 3, security: 2, pricing: 2, integrations: 2, technical: 1,
};
      ...
      <section className="page-tint">
        <div className="mx-auto max-w-[1280px] px-6 lg:px-10 pt-14 lg:pt-20 pb-10 text-center">
          ...
          <p className="mt-5 text-ink-500 text-[15px] max-w-[640px] mx-auto leading-relaxed">
            Find answers to common questions about AI HRMS, our features,
                ...
                {CATEGORIES.map((c) => (
                  <li key={c.id}>
                    <button
                      type="button"
                      onClick={() => setCat(c.id)}
                      className={`w-full flex items-center justify-between gap-2 px-3 py-2.5 rounded-lg text-[13px] transition ${
                        cat === c.id
                    ...
                    >
                      <span className="flex items-center gap-2">
                        <span className={cat === c.id ? "text-brand-600" : "text-ink-400"}>{c.icon}</span>
                        {c.title}
                      </span>
                      <span className={`text-[11px] font-bold px-2 py-0.5 rounded ${
                        cat === c.id ? "bg-brand-100 text-brand-700" : "bg-ink-100 text-ink-500"
                      }`}>
                        {COUNTS[c.id]}
                      </span>
                    </button>
            ...
            <div className="card p-5 mt-4 bg-brand-50 border-brand-100">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <div className="font-display font-bold text-[14px]">Still have questions?</div>
            ...
            {filtered.map((q, i) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span className={`font-semibold text-[14px] w-7 ${open ? "text-brand-600" : "text-ink-400"}`}>
                      {i + 1}.
                    </span>
                    <span className={`flex-1 font-semibold text-[15px] ${open ? "text-brand-700" : "text-ink-900"}`}>
                      {q.q}
                    </span>
                    <span className={`w-7 h-7 rounded-full border flex items-center justify-center text-[14px] transition ${
                      open ? "border-brand-300 text-brand-600" : "border-ink-200 text-ink-500"
                    }`}>
                      {open ? "−" : "+"}
                    </span>
                  </button>
