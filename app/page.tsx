...
/* ============================================================
   HERO DIAGRAM — three columns with dotted SVG connectors
   ============================================================ */
function Diagram() {
  return (
    <div className="relative">
      {/* Layout grid — 4 columns so the Hiring Time badge has its own
          dedicated space and never overlaps the workflow rail.
          (SVG arrow overlay removed — it was misaligning at different
          viewport widths. The visual flow reads clearly from the layout itself.) */}
      <div className="relative z-10 grid grid-cols-[0.85fr_1.2fr_1.1fr_0.5fr] gap-5 items-center">
        {/* COL 1 — three floating badges */}
        ...
        <div className="flex flex-col gap-3 justify-between h-[460px] py-2">
          ...
          <RailItem
            icon="check"
            title={<span><strong className="font-extrabold">98% Match</strong> Found</span>}
            sub="Best fit for this role"
            highlight
          />
        </div>

        {/* COL 4 — Hiring Time pill, vertically centered next to the rail */}
        <div className="flex items-center justify-start">
          <div className="card px-3 py-2.5 flex items-center gap-2.5 shadow-[0_18px_50px_-20px_rgba(15,23,42,0.18)]">
            <span className="w-8 h-8 rounded-md bg-green-50 text-green-600 flex items-center justify-center shrink-0">
              <TrendIcon />
            </span>
            <div className="leading-tight">
...
function CandidateCard() {
  return (
    <div className="card p-6 text-center shadow-[0_24px_60px_-24px_rgba(109,76,255,0.45)]">
      <div className="mx-auto w-[88px] h-[88px] rounded-full illus-placeholder text-[10px] !min-h-[88px]">
        [photo]
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5 justify-center">
        {["Java", "Spring Boot", "AWS", "Kubernetes"].map((s) => (
          <span key={s} className="text-[10px] px-2 py-0.5 rounded-full bg-brand-50 text-brand-700 border border-brand-100 font-semibold">
            {s}
          </span>
        ))}
      </div>
      ...
      <Link
        href="/candidate/sarah"
        className="mt-5 inline-flex items-center justify-center gap-2 w-full py-3 rounded-xl text-white text-[12.5px] font-semibold shadow-sm hover:opacity-95 transition"
        style={{ background: "var(--gradient-brand)" }}
      >
...
function RailItem({
  ...
  icon: "doc" | "brain" | "users" | "chart" | "check";
...
function CircularScore({ value }: { value: number }) {
  ...
  return (
    <svg width="110" height="110" viewBox="0 0 110 110" className="block mx-auto my-1">
      ...
      />
      <text x="55" y="63" textAnchor="middle" fontSize="24" fontWeight="800" fill="#0F172A" fontFamily="var(--font-display)">
        {value}%
      </text>
    </svg>
  );
