...
| `/company/clients` | Clients list — 3 stat tiles (Total/Active/Open Jobs), client-side search + Industry/Status/Account-Manager filters, table (Client Name/Industry/Account Manager/Open Jobs/Last Activity/Status/Actions), empty state, footer `<Pagination>` + rows-per-page; rail (Client Overview donut w/ live status split, Top Clients, Recent Additions from latest clients, Quick Actions). Sidebar "Clients" links here; "Add New Client" → `/company/clients/new` | ✅ Built + API wired (`listClients` → `GET /company/clients`) |
| `/company/clients/new` | Add New Client — 3-section form (Company Information, Primary Contact, Billing) + rail (Why Add a Client / Information Guide / What's Next); breadcrumb + Cancel return to `/company/clients` | ✅ Built + API wired (`createClient` → `POST /company/clients`) |
...
| `GET` | `/company/clients?page=&size=` | Paginated `ClientSummaryResponse` list |
